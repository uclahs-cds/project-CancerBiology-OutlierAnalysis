...
# input variable : data.fpkm - raw FPKM data
...
molecular.data <- data.fpkm[, patient.part];
...
quantify.outliers <- function(x, method = 'mean', trim = 0, exclude.zero = FALSE) {
    ...
    if (any(is.na(x.num))) {
        ...
        if ('median' == method) {
            ...
            (x.num - x.median) / x.mad;
            }
        else if ('kmean' == method) {
            if (exclude.zero) {
                if (length(unique(as.numeric(x.num))) == 1) {
                    kmean.matrix <- rep(NA, length(x.num));
                    names(kmean.matrix) <- names(x);
                    } 
                else {
                    data.order <- sort(x.num, decreasing = TRUE);
                    ...
                    if (length(unique(as.numeric(non.zero))) <= 2) {
                        ...
                        kmean.matrix <- cluster.zero[match(x.num, data.order)];
                        names(kmean.matrix) <- names(x);  
                        } 
                    else {
...
data.mean <- apply(molecular.data, 1, quantify.outliers);
...
data.mean.nonzero <- apply(molecular.data, 1, quantify.outliers, exclude.zero = TRUE);
...
data.trimmean <- apply(molecular.data, 1, quantify.outliers, trim = 5);
...
data.trimmean.nonzero <- apply(molecular.data, 1, quantify.outliers, trim = 5, exclude.zero = TRUE);
...
data.median <- apply(molecular.data, 1, quantify.outliers, method = 'median');
...
data.median.nonzero <- apply(molecular.data, 1, quantify.outliers, method = 'median', exclude.zero = TRUE);
...
data.kmean <- apply(molecular.data, 1, quantify.outliers, method = 'kmean');
data.kmean.t <- t(data.kmean);
data.kmean.t <- data.frame(data.kmean.t);

# 8. KMEAN excluding zero : method = 'kmean', exclude.zero = TRUE
data.kmean.nonzero <- apply(molecular.data, 1, quantify.outliers, method = 'kmean', exclude.zero = TRUE);
...
mean.zscore <- cbind(data.zrange.mean.t, data.fpkm$Symbol);
colnames(mean.zscore) <- c(colnames(data.zrange.mean.t), 'Symbol');

# 2. MEAN and SD excluding zero
data.zrange.mean.nonzero <- apply(data.mean.nonzero.t, 1, outlier.detection.zrange);
data.zrange.mean.nonzero.t <- data.frame(t(data.zrange.mean.nonzero));
mean.nonzero.zscore <- cbind(data.zrange.mean.nonzero.t, data.fpkm$Symbol);
colnames(mean.nonzero.zscore) <- c(colnames(data.zrange.mean.nonzero.t), 'Symbol');
...
trimmean.zscore <- cbind(data.zrange.trimmean.t, data.fpkm$Symbol);
colnames(trimmean.zscore) <- c(colnames(data.zrange.trimmean.t), 'Symbol');

# 4. TRIMMED MEAN and TRIMMED SD excluding zero
data.zrange.trimmean.nonzero <- apply(data.trimmean.nonzero.t, 1, outlier.detection.zrange);
data.zrange.trimmean.nonzero.t <- data.frame(t(data.zrange.trimmean.nonzero));
trimmean.nonzero.zscore <- cbind(data.zrange.trimmean.nonzero.t, data.fpkm$Symbol);
colnames(trimmean.nonzero.zscore) <- c(colnames(data.zrange.trimmean.nonzero.t), 'Symbol');

# 5. MEDIAN and MAD
data.zrange.median <- apply(data.median.t, 1, outlier.detection.zrange);
data.zrange.median.t <- data.frame(t(data.zrange.median));
median.zscore <- cbind(data.zrange.median.t, data.fpkm$Symbol);
colnames(median.zscore) <- c(colnames(data.zrange.median.t), 'Symbol');

# 6. MEDIAN and MAD excluding zero
data.zrange.median.nonzero <- apply(data.median.nonzero.t, 1, outlier.detection.zrange);
data.zrange.median.nonzero.t <- data.frame(t(data.zrange.median.nonzero));
median.nonzero.zscore <- cbind(data.zrange.median.nonzero.t, data.fpkm$Symbol);
colnames(median.nonzero.zscore) <- c(colnames(data.zrange.median.nonzero.t), 'Symbol');
...
kmean.fraction <- cbind(data.fraction.kmean.t, data.fpkm$Symbol);
colnames(kmean.fraction) <- c(colnames(data.fraction.kmean.t), 'Symbol');

# 8. KMEAN fraction excluding zero
data.fraction.kmean.nonzero <- apply(data.kmean.nonzero.t, 1, outlier.detection.kmean);
data.fraction.kmean.nonzero.t <- data.frame(t(data.fraction.kmean.nonzero));
kmean.nonzero.fraction <- cbind(data.fraction.kmean.nonzero.t, data.fpkm$Symbol);
colnames(kmean.nonzero.fraction) <- c(colnames(data.fraction.kmean.nonzero.t), 'Symbol');
...
for (i in 1:length(data.list)) {
    patient.match.check(data.fpkm[,patient.part], data.list[[i]][,patient.part]);
    }
...
outlier.heatmap.fpkm(median.zscore, data.fpkm);
...
# p-value 
p.value <- qnorm(0.05/2, lower.tail=FALSE);
outlier.waterfallplot(median.zscore, data.fpkm, plot.num = 1, threshold = p.value);
...
outlier.waterfallplot(median.zscore, data.fpkm, plot.num = 1, threshold = bon.zscore);
...
outlier.waterfallplot(median.zscore, data.fpkm, plot.num = 1, threshold = adjust.p.zscore);


### K-mean matrix in waterfall plot
outlier.waterfallplot(kmean.fraction, data.fpkm, plot.num = 1, kmean = TRUE);
...
outlier.technical.problem <- function(x) {
    ...
    gene.position.all <- biomaRt:::getBM(attributes = c('ensembl_gene_id', 'hgnc_symbol', 'chromosome_name',
                                         ...
                                         mart = ensembl.all);
    gene.all <- subset(data.fpkm, gene.list.all.sub %in% gene.position.all$ensembl_gene_id);
...
outlier.heatmap.rank <- function(x) {
    fpkm <- data.fpkm[rownames(data.fpkm) %in% rownames(x)[1:1000],];
    fpkm.log <- log10((fpkm[ , 1:(length(fpkm)-1)])+1);
    fpkm.heat <- BoutrosLab.plotting.general:::create.heatmap(
        x = fpkm.log,
        clustering.method = 'ward.D2',
...
# Match the patients between data.fpkm vs clinical.data
clinical.data$Patient.ID <- gsub('-', '.', clinical.data$Patient.ID);
clinical.data.match <- clinical.data[clinical.data$Patient.ID %in% colnames(data.fpkm[,patient.part]),];
...
clinical.outcome <- clinical.data.match.dup[match(colnames(data.fpkm[,patient.part]), clinical.data.match.dup$Patient.ID),];
...
for (i in 1:length(primary.gs)) {
    ...
    else if (primary.gs[i] == 3 & secondary.gs[i] == 4) {
        isup[i,1] <- 2;
        }
    else if (primary.gs[i] == 4 & secondary.gs[i] == 3) {
        isup[i,1] <- 3;
        }
    else if (final.gs[i] == 8) {
        isup[i,1] <- 4;
        }
...
for (i in 1:length(grade.sub)) {
    ...
    else if (grade.sub[i] == 'T2') {
        grade.sub.stage[i,1] <- 15;
        }
    else if (grade.sub[i] == 'T3') {
        grade.sub.stage[i,1] <- 16;
        }
    else if (grade.sub[i] == 'T4') {
...
rownames(all.clinical) <- colnames(data.fpkm[,patient.part]);
...
outlier.multiwaterfallplot(data.fpkm, gene.rank.order.6, mean.zscore, mean.nonzero.zscore,
...
for (i in 1:length(data.list)) {
    data.match.check(data.fpkm[,patient.part], data.list[[i]][,patient.part]);
    }
