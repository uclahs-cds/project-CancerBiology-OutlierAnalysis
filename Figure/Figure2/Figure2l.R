...
i.me.patient <- i.me[, which(i.patient == 1)];
i.me.patient.non <- i.me[, which(i.patient == 0)];
...
promoters.i <- promoters[rownames(i.me), ];
...
fpkm.i.order <- fpkm.i.order[
    , 
    order(fpkm.i.order[1,], decreasing = TRUE), 
    drop = FALSE
    ];
...
i.heat  <- BoutrosLab.plotting.general:::create.multiplot(
    plot.objects = list(i.heat.out.non, i.heat.out),
    x.relation = "sliced",
    y.relation = "sliced",
    main = i,
