...
cpcg.fpkm <- read.delim("~/Documents/1.Project/1.CPCGENE_RNA-seq/Rawdata/2018-01-23_rnaseq_rsem_gene_FPKM.txt", row.names = 1);
...
#};
#zscore.bymedian.symbol <- cbind(data.frame(cpcg.fpkm$row.names),zscore.bymedian, data.frame(cpcg.fpkm$Symbol), patient.median, patient.mad);
#colnames(zscore.bymedian.symbol) <- c(colnames(cpcg.fpkm), "Median", "mad");
#write.table(zscore.bymedian.symbol, file = 'cpcg.zscore.bymedian.txt', sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
cpcg.zscore.bymedian <- read.delim("~/Documents/1.Project/1.CPCGENE_RNA-seq/Rawdata/cpcg.zscore.bymedian.txt", row.names = 1);
...
for (i in 1:100) {
   ...
   nonzero.fpkm <- round(sum(ifelse(fpkm.b$gene == 0, 0, 1))/144*100, 2)
...
#};
#zscore.bytrimmean.symbol <- cbind(data.frame(cpcg.fpkm$row.names),zscore.bytrimmean, data.frame(cpcg.fpkm$Symbol), patient.trimmean, patient.trimsd);
#colnames(zscore.bytrimmean.symbol) <- c(colnames(cpcg.fpkm), "Trimmean", "Trimsd");
#write.table(zscore.bytrimmean.symbol, file = 'cpcg.zscore.bytrimmean.txt', sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
cpcg.zscore.bytrimmean <- read.delim("~/Documents/1.Project/1.CPCGENE_RNA-seq/Rawdata/cpcg.zscore.bytrimmean.txt", row.names = 1);
