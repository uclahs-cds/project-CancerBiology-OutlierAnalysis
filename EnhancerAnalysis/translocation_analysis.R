...
# 0.01		2024-04-04	jlivingstone	initial code
...
trans$submitted_sample_id <- sub('a2', 'a', trans$submitted_sample_id)
trans$submitted_sample_id <- sub('a3', 'a', trans$submitted_sample_id)

samples.with.muts <- unique(trans$submitted_sample_id)
...
for (j in 1:nrow(true.outliers)) {
	...
	muts <- trans[trans$submitted_sample_id %in% true.outliers$sample.name[j],]
	chr.muts <- muts[which(muts$chr_to == gene.annot$chr),]
	...
	if (gene.annot$strand == '+') {
		...
		# upstream is to the left, translocation to be in window
		trans.ranges <- GRanges(
		...
		# doesn't make sense for genes on '-' strand since enhancer elements aren't changed
		results[[j]] <- 'Gene on negative strand'
		next
		}
	...
	overlap <- findOverlaps(trans.ranges, gene.ranges, type = 'within')
	if (length(overlap) == 0) {
		results[[j]] <- 'No overlap with translocation'
		next
		}
	...
	# translocation event that brings elements together
...
save(
	results,
	file = generate.filename('outlier', paste0(bp.threshold, 'bp_translocation_analysis'), 'rda')
	)
